		...
		struct omap_display *disp, int level)
{
	...
	printk(KERN_DEBUG "panel_set_backlight_level [%s] %i\n",
...
static struct omap_display_data tpo_wvga_48_panel = {
	.type = OMAP_DISPLAY_TYPE_DPI,
	.name = "lcd",
	.panel_name = "tpo_wvga_48",
	.u.dpi.data_lines=24,
...
int __init panel_tpo_wvga_48_init(struct omap_dss_platform_data *disp_data)
{
	...
	if (ret < 0) {
		pr_err("panel_tpo_wvga_48_init: cannot register panel spi\n");
		return ret;
	}
	
	panel_init(&tpo_wvga_48_panel);
	disp_data->displays[disp_data->num_displays] = &tpo_wvga_48_panel;
