...
//		printk(KERN_ERR "No free ch in the chain\n");
		return -EBUSY;
	}
...
void omap_dma_disable_irq(int lch)
{
	...
	if (cpu_class_is_omap2()) {
		/* Disable interrupts */
		val = omap_readl(OMAP34XX_DMA4_BASE + OMAP_DMA4_IRQENABLE_L0);
		val &= ~(1 << lch);
		omap_writel(val, OMAP34XX_DMA4_BASE + OMAP_DMA4_IRQENABLE_L0);
	}
}
