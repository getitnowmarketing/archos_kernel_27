		...
		struct omap_display *display)
{
	...
	if (dw < info->pos_x + outw) {
		DSSDBG("check_overlay failed 1 (dw:%i info->pos_x:%i outw:%i)\n",
				dw, info->pos_x, outw);
		return -EINVAL;
	}

	if (dh < info->pos_y + outh) {
		DSSDBG("check_overlay failed 2 (dh:%i info->pos_y:%i outh:%i)\n",
				dh, info->pos_y, outh);
		return -EINVAL;
	}
...
/* do not check overlay here, the check depends on values that are only set with 
   omap_dss_setup_overlay_output() below, so the check might fail although the final
   config is OK!
...
static void configure_manager(enum omap_channel channel)
{
	...
	omap_dispc_set_trans_key(channel, mgr->info.trans_key_type, mgr->info.trans_key);
	omap_dispc_enable_trans_key(channel, mgr->info.trans_enabled);
	dispc_enable_alpha_blending(channel, mgr->info.alpha_enabled);
...
static int omap_dss_mgr_apply(struct omap_overlay_manager *mgr)
{
	...
	for (i = 0; i < mgr->num_overlays; i++) {
		...
		if (!ovl->info.enabled || !ovl->info.paddr) {
			dispc_enable_plane(ovl->id, 0);
			continue;
		}
		...
		/* if we have an interlaced panel connected, some software
		 * support is needed */
		if (mgr->display->panel->timings.interlaced)
			ilace = 1;
	...
	for (i=0; i<omap_dss_get_num_overlay_managers() ;i++) {
		...
		struct omap_overlay_manager *mgr_entry;
		mgr_entry = omap_dss_get_overlay_manager(i);
		 
		if (!(mgr_entry->caps & OMAP_DSS_OVL_MGR_CAP_DISPC))
			continue;

		if (!mgr_entry->info_dirty)
			continue;
...
static struct omap_overlay_manager dispc_overlay_managers[] =
{
	[OMAP_DSS_OVL_MGR_LCD] = {
		...
		.set_manager_info = &omap_dss_mgr_set_info,
		.get_manager_info = &omap_dss_mgr_get_info,
