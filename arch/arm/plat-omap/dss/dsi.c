		...
		struct dsi_clock_info *cinfo)
{
	...
	DSSDBG("dsi_pll_calc_pck is_tft:%d req_pck:%ld\n", is_tft, req_pck);
...
int dsi_pll_init(int enable_hsclk, int enable_hsdiv)
{
	...
	r = dispc_calc_clock_div(1, 24 * 1000 * 1000, &cinfo);
	if (r)
		return r;
...
static int dsi_display_enable(struct omap_display *display)
{
	...
	r = omap_dispc_register_isr(framedone_callback, NULL, DISPC_IRQ_FRAMEDONE);
	if (r) {
