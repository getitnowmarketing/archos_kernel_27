...
static struct gpio_restore_regs gpio_restore_banks[5];
#endif
...
static void gpio_mask_irq(unsigned int irq)
{
	...
	_set_gpio_wakeup(bank, get_gpio_index(gpio), 0);
	_set_gpio_irqenable(bank, gpio, 0);
}
...
static void gpio_unmask_irq(unsigned int irq)
{
	...
	_set_gpio_wakeup(bank, get_gpio_index(gpio), 1);
}
...
static int __init _omap_gpio_init(void)
{
	...
	if (cpu_is_omap34xx()) {
		for (i = 0; i < OMAP34XX_NR_GPIOS; i++) {
			...
			gpio_iclks[i] = clk_get(dev, clk_name);
	...
	for (i = 0; i < gpio_bank_count; i++) {
		...
		if (cpu_is_omap34xx()) {
			...
			bank->dbck = clk_get(dev, clk_name);
			if (IS_ERR(bank->dbck))
				printk(KERN_ERR "Could not get %s\n", clk_name);
		}
	}
...
void omap_gpio_save(void)
{
	...
	for (i = 0; i < gpio_bank_count - 1; i++) {
		struct gpio_bank *bank = &gpio_bank[i + 1];
