...
{
	unsigned long unplug_thresh = (unsigned long)q->unplug_thresh;
	ssize_t ret = queue_var_store(&unplug_thresh, page, count);

	if (unplug_thresh < 1 || unplug_thresh > 16)
		return -EINVAL;

	spin_lock_irq(q->queue_lock);
	q->unplug_thresh = (int)unplug_thresh;
	spin_unlock_irq(q->queue_lock);
...
{
	unsigned int unplug_thresh = (unsigned int)q->unplug_thresh;

	return queue_var_show(unplug_thresh, (page));
}
