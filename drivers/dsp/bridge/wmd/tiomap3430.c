				...
				u32 dwDSPAddr)
{
	...
	if (DSP_SUCCEEDED(status)) {
		...
		CM_AUTOIDLE_PLL_IVA2 &= ~OMAP3430_AUTO_IVA2_DPLL_MASK;
		CM_AUTOIDLE_PLL_IVA2 |= 1 << OMAP3430_AUTO_IVA2_DPLL_SHIFT;
		...
		CM_CLKSTCTRL_IVA2 = 0x00000003; /* Allow automatic transition */
				 ...
				 IN CONST struct CFG_DSPRES *pDspConfig)
{
	...
	/* Assuming that we use only DSP's memory map until 0x4000:0000,
	 * we would need only 1024 L1 enties i.e L1 size = 4K */
	...
	pPtAttrs->L2size = HW_MMU_COARSE_PAGE_SIZE * pPtAttrs->L2NumPages;
