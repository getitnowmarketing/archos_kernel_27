...
void DSPClkWakeupEventCtrl(u32 ClkId, bool enable)
{
	...
	case BPWR_GPTimer5:
		if (enable) {
			PM_IVA2GRPSEL_PER |= OMAP3430_GRPSEL_GPT5;
			PM_MPUGRPSEL_PER &= ~OMAP3430_GRPSEL_GPT5;
		} else {
			PM_MPUGRPSEL_PER |= OMAP3430_GRPSEL_GPT5;
			PM_IVA2GRPSEL_PER &= ~OMAP3430_GRPSEL_GPT5;
		}
	...
	case BPWR_GPTimer6:
		if (enable) {
			PM_IVA2GRPSEL_PER |= OMAP3430_GRPSEL_GPT6;
			PM_MPUGRPSEL_PER &= ~OMAP3430_GRPSEL_GPT6;
		} else {
			PM_MPUGRPSEL_PER |= OMAP3430_GRPSEL_GPT6;
			PM_IVA2GRPSEL_PER &= ~OMAP3430_GRPSEL_GPT6;
		}
	...
	case BPWR_GPTimer7:
		if (enable) {
			PM_IVA2GRPSEL_PER |= OMAP3430_GRPSEL_GPT7;
			PM_MPUGRPSEL_PER &= ~OMAP3430_GRPSEL_GPT7;
		} else {
			PM_MPUGRPSEL_PER |= OMAP3430_GRPSEL_GPT7;
			PM_IVA2GRPSEL_PER &= ~OMAP3430_GRPSEL_GPT7;
		}
	...
	case BPWR_GPTimer8:
		if (enable) {
			PM_IVA2GRPSEL_PER |= OMAP3430_GRPSEL_GPT8;
			PM_MPUGRPSEL_PER &= ~OMAP3430_GRPSEL_GPT8;
		} else {
			PM_MPUGRPSEL_PER |= OMAP3430_GRPSEL_GPT8;
			PM_IVA2GRPSEL_PER &= ~OMAP3430_GRPSEL_GPT8;
		}
	...
	case BPWR_MCBSP1:
		if (enable) {
			PM_IVA2GRPSEL1_CORE |= OMAP3430_GRPSEL_MCBSP1;
			PM_MPUGRPSEL1_CORE &= ~OMAP3430_GRPSEL_MCBSP1;
		} else {
			PM_MPUGRPSEL1_CORE |= OMAP3430_GRPSEL_MCBSP1;
			PM_IVA2GRPSEL1_CORE &= ~OMAP3430_GRPSEL_MCBSP1;
		}
	...
	case BPWR_MCBSP2:
		if (enable) {
			PM_IVA2GRPSEL_PER |= OMAP3430_GRPSEL_MCBSP2;
			PM_MPUGRPSEL_PER &= ~OMAP3430_GRPSEL_MCBSP2;
		} else {
			PM_MPUGRPSEL_PER |= OMAP3430_GRPSEL_MCBSP2;
			PM_IVA2GRPSEL_PER &= ~OMAP3430_GRPSEL_MCBSP2;
		}
	...
	case BPWR_MCBSP3:
		if (enable) {
			PM_IVA2GRPSEL_PER |= OMAP3430_GRPSEL_MCBSP3;
			PM_MPUGRPSEL_PER &= ~OMAP3430_GRPSEL_MCBSP3;
		} else {
			PM_MPUGRPSEL_PER |= OMAP3430_GRPSEL_MCBSP3;
			PM_IVA2GRPSEL_PER &= ~OMAP3430_GRPSEL_MCBSP3;
		}
	...
	case BPWR_MCBSP4:
		if (enable) {
			PM_IVA2GRPSEL_PER |= OMAP3430_GRPSEL_MCBSP4;
			PM_MPUGRPSEL_PER &= ~OMAP3430_GRPSEL_MCBSP4;
		} else {
			PM_MPUGRPSEL_PER |= OMAP3430_GRPSEL_MCBSP4;
			PM_IVA2GRPSEL_PER &= ~OMAP3430_GRPSEL_MCBSP4;
		}
	...
	case BPWR_MCBSP5:
		if (enable) {
			PM_IVA2GRPSEL1_CORE |= OMAP3430_GRPSEL_MCBSP5;
			PM_MPUGRPSEL1_CORE &= ~OMAP3430_GRPSEL_MCBSP5;
		} else {
			PM_MPUGRPSEL1_CORE |= OMAP3430_GRPSEL_MCBSP5;
			PM_IVA2GRPSEL1_CORE &= ~OMAP3430_GRPSEL_MCBSP5;
		}
				  ...
				  IN void *pArgs)
{
	...
	return DSP_SOK;
#else
	if (dsp_constraint_handle && constraint_set(dsp_constraint_handle,
