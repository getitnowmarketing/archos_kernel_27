...
#define RX_FREE_WAIT	// to separate tx/rx spi cycles
...
static int ads7846_read12_ser(struct device *dev, unsigned command)
{
	...
	if (ts->model == 2008)
		req->xfer[2].delay_usecs = 100;

	spi_message_add_tail(&req->xfer[2], &req->msg);

	req->xfer[3].rx_buf = &req->sample;
	req->xfer[3].tx_buf = &padd;
	req->xfer[3].len = 2;
	...
	if (ts->model == 2008)
		req->xfer[4].delay_usecs = 100;

	spi_message_add_tail(&req->xfer[4], &req->msg);

	req->xfer[5].rx_buf = &req->scratch;
	req->xfer[5].tx_buf = &padd;
	req->xfer[5].len = 2;
...
#ifdef RX_FREE_WAIT
static void tsc_tx_wait (void * ads)
{
	struct ads7846 *ts = ads;
...
static void ads7846_disable(struct ads7846 *ts)
{
	...
	if (!ts->pending) {
		...
		omap_set_gpio_debounce(irq_to_gpio(ts->spi->irq), 0);
	} else {
...
static void ads7846_enable(struct ads7846 *ts)
{
	...
	ts->disabled = 0;
	ts->irq_disabled = 0;
	omap_set_gpio_debounce(irq_to_gpio(ts->spi->irq), 1);
	enable_irq(ts->spi->irq);
}
...
#else
static void ads7846_early_suspend(struct early_suspend *es)
{
	struct ads7846 *ts = container_of(es, struct ads7846, early_suspend);
...
static void ads7846_early_resume(struct early_suspend *es)
{
	struct ads7846 *ts = container_of(es, struct ads7846, early_suspend);
...
static int __devinit ads7846_probe(struct spi_device *spi)
{
...
#ifndef RX_FREE_WAIT
	if (pdata->settle_delay_usecs && (ts->model == 2008) )
		x->delay_usecs = pdata->settle_delay_usecs;
#else
	m->complete = tsc_tx_wait;
	m->context = ts;
	...
	if ( (ts->model == 7846) || (ts->model == 2008)) {
		m++;
		spi_message_init(m);
	 ...
	 */
	if (ts->model != 2008)
		(void) ads7846_read12_ser(&spi->dev, READ_12BIT_SER(vaux) | ADS_PD10_ALL_ON);
	else
		(void) ads7846_read12_ser(&spi->dev, READ_Y(0));
	...
	ts->early_suspend.resume = ads7846_early_resume;
