...
static int synaptics_init_panel(struct synaptics_ts_data *ts)
{
	...
	if (ret < 0) {
		printk(KERN_ERR "i2c_smbus_write_byte_data \
	...
	for (i = 0; i < ((ts->use_irq && !bad_data) ? 1 : 10); i++) {
		...
		if (ret < 0) {
			...
			if ((buf[14] & 1) == 0)
				...
				int f, a;
				int base;
				/* int x = buf[3] | (uint16_t)(buf[2]
						  & 0x1f) << 8; */
				/* int y = buf[5] | (uint16_t)(buf[4]
						  & 0x1f) << 8; */
				int z = buf[1];
				int w = buf[0] >> 4;
				int finger = buf[0] & 7;

				/* int x2 = buf[3+6] | (uint16_t)(buf[2+6]
						     & 0x1f) << 8; */
				/* int y2 = buf[5+6] | (uint16_t)(buf[4+6]
						     & 0x1f) << 8; */
				/* int z2 = buf[1+6]; */
				/* int w2 = buf[0+6] >> 4; */
				...
				/* printk("x %4d, y %4d, z %3d, w %2d, F %d,
					  2nd: x %4d, y %4d, z %3d, w %2d,
					  F %d, dx %4d, dy %4d\n", */
				/*	x, y, z, w, finger, */
				/*	x2, y2, z2, w2, finger2, */
				/*	dx, dy); */
...
printk(KERN_INFO "panel version = %x\n", panel_version);
if (pdata) {
...
if (ts->input_dev == NULL) {
	ret = -ENOMEM;
	printk(KERN_ERR "synaptics_ts_probe: Failed to \
...
printk(KERN_INFO "synaptics_ts_probe: inactive_x %d %d, \
		inactive_y %d %d\n", inactive_area_left, \
		inactive_area_right, inactive_area_top, \
		inactive_area_bottom);
printk(KERN_INFO "synaptics_ts_probe: snap_x %d-%d %d-%d, \
	snap_y %d-%d %d-%d\n",
...
if (ret) {
	printk(KERN_ERR "synaptics_ts_probe: Unable to register %s \
			input device\n", ts->input_dev->name);
...
/* TODO: Code cleanup */
#if 0
i2c_smbus_write_byte_data(ts->client, 0x00, 0x00); /* enable abs int */
#endif
if (client->irq) {
	...
	/* This is need for intial start transfer to trigger furhter Interrupts: TI ZOOM2 */
msg[0].addr = ts->client->addr;
msg[0].flags = 0;
msg[0].len = 1;
msg[0].buf = buf0;
