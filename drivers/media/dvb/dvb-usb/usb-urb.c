...
static int usb_allocate_stream_buffers(struct usb_data_stream *stream, int num, unsigned long size, int type)
{
	stream->buf_num = 0;
	...
	for (stream->buf_num = 0; stream->buf_num < num; stream->buf_num++) {
		...
		if (type == USB_BULK) {
			stream->buf_list[stream->buf_num] = kmalloc(size, GFP_KERNEL);
			stream->dma_addr[stream->buf_num] = 0;			
		} else {
			stream->buf_list[stream->buf_num] = usb_buffer_alloc(stream->udev, size, GFP_KERNEL,
...
static int usb_bulk_urb_init(struct usb_data_stream *stream)
{
	...
	if ((i = usb_allocate_stream_buffers(stream,stream->props.count,
					stream->props.u.bulk.buffersize, USB_BULK)) < 0)
		return i;
...
static int usb_isoc_urb_init(struct usb_data_stream *stream)
{
	...
	if ((i = usb_allocate_stream_buffers(stream,stream->props.count,
					stream->props.u.isoc.framesize*stream->props.u.isoc.framesperurb, USB_ISOC)) < 0)
		return i;
