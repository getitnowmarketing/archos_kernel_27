...
struct dib7000p_state {
	...
	int num_demod_slaves;
	struct dvb_frontend *demod_slaves[MAX_NUM_SLAVES];
	...
	unsigned int sfn_workaround_active;
};
...
typedef enum { DIV_INTERNAL=0, DIV_BOTH, DIV_INPUT } div_onoff_t;
static int dib7000p_set_diversity_in(struct dvb_frontend *demod, div_onoff_t onoff)
{
...
//printk("dib7000p_set_diversity_in: div_sync_wait: %d\n", state->div_sync_wait);
		dib7000p_write_word(state, 207, (state->div_sync_wait << 4) | (1 << 2) | (2 << 0));
	}
	state->div_state = onoff;

	switch (onoff) {
		case DIV_INTERNAL: /* only use the internal way - not the diversity input */
			dib7000p_write_word(state, 204, 1);
			dib7000p_write_word(state, 205, 0);
			break;
		case DIV_BOTH: /* both ways */
			dib7000p_write_word(state, 204, 6);
			dib7000p_write_word(state, 205, 16);
			break;
		case DIV_INPUT: /* only the diversity input */
...
static void dib7000p_set_channel(struct dib7000p_state *state, struct dvb_frontend_parameters *ch, u8 seq)
{
	...
	state->div_sync_wait = (value * 3) / 2 + 48; // add 50% SFN margin + compensate for one DVSY-fifo (AK: was +32 - Dibcom ref. +48)
...
static int dib7000pd_wakeup(struct dvb_frontend *demod);
static int dib7000pd_sleep(struct dvb_frontend *demod);

static int dib7000p_wakeup(struct dvb_frontend *demod)
{
	struct dib7000p_state *state = demod->demodulator_priv;
	if (state->num_demod_slaves > 0)
		return dib7000pd_wakeup(demod);
...
static int dib7000pd_get_frontend(struct dvb_frontend* fe,
				struct dvb_frontend_parameters *fep)
{
	struct dib7000p_state *state = fe->demodulator_priv;
	
	if (state->num_demod_slaves > 0)
		return dib7000p_get_frontend(state->demod_slaves[state->worked], fep);
	
	return dib7000p_get_frontend(fe, fep);
}

static int dib7000pd_set_frontend(struct dvb_frontend* fe,
				struct dvb_frontend_parameters *fep);
...
dprintk("%s\n", __FUNCTION__);
	...
	if (state->num_demod_slaves > 0)
	{
		return dib7000pd_set_frontend(fe, fep);
	}	
...
static int dib7000p_read_status(struct dvb_frontend *fe, fe_status_t *stat)
{
	...
	if (state->num_demod_slaves > 0)
		state = state->demod_slaves[state->worked]->demodulator_priv;

	lock = dib7000p_read_word(state, 509);
...
static int dib7000p_read_ber(struct dvb_frontend *fe, u32 *ber)
{
	...
	if ( lock & 0x0040 )  
		*ber = ((dib7000p_read_word(state, 500) << 16) | dib7000p_read_word(state, 501)) & 0x1FFFFF; 
	else 
		*ber = 20000; /* chip signals a high BER - use 2e-4 */ 
	return 0;
}
...
static int dib7000p_read_snr(struct dvb_frontend* fe, u16 *snr)
{
	struct dib7000p_state *state = fe->demodulator_priv;
	u16 val;
	u32 equal_noise_mant, equal_signal_mant;
	s16 equal_noise_exp, equal_signal_exp;
	u32 cn = 0;
	...
	if ((equal_noise_exp & 0x20) != 0)
		equal_noise_exp -= 0x40;

	equal_signal_mant = (val >> 6) & 0xFF;
	equal_signal_exp  = (val & 0x3F);
	if ((equal_signal_exp & 0x20) != 0)
		equal_signal_exp -= 0x40;

	if (equal_noise_mant != 0) {		
		if ( equal_signal_exp > equal_noise_exp )
			cn = ( equal_signal_mant * ( 1ul << ( equal_signal_exp - equal_noise_exp ) )) / equal_noise_mant ;
		 else {
		 	u32 num = equal_noise_mant * ( 1ul << ( equal_noise_exp - equal_signal_exp ));
			if ( num != 0 )
				cn = equal_signal_mant / num;
		}
	}	
	*snr = ( cn < 0xFFFF ) ? cn : 0xFFFF;
	return 0;
}
...
struct dvb_frontend * dib7000p_attach(struct i2c_adapter *i2c_adap, u8 i2c_addr, struct dib7000p_config *cfg)
{
	 ...
	 */
#if 0
AK: This was not in our 2.6.22 driver - and might kill diversity, please check!!!
...
static struct dvb_frontend_ops dib7000p_ops = {
	...
	.get_frontend         = dib7000pd_get_frontend,
...
static int dib7000pd_wakeup(struct dvb_frontend *demod)
{
	int i;
	struct dib7000p_state *state = demod->demodulator_priv;
	for (i=0; i<state->num_demod_slaves; i++)
	{
		dib7000p_set_power_mode(state->demod_slaves[i]->demodulator_priv, DIB7000P_POWER_ALL);
		dib7000p_set_adc_state(state->demod_slaves[i]->demodulator_priv, DIBX000_SLOW_ADC_ON);
	}
...
static int dib7000pd_sleep(struct dvb_frontend *demod)
{
	...
	struct dib7000p_state *state = demod->demodulator_priv;
	for (i=0; i<state->num_demod_slaves; i++)
		ret |= dib7000p_set_output_mode(state->demod_slaves[i]->demodulator_priv, OUTMODE_HIGH_Z) | dib7000p_set_power_mode(state->demod_slaves[i]->demodulator_priv, DIB7000P_POWER_INTERFACE_ONLY);

	return ret;
}

#define MAX_AUTOSEARCH_ITERATIONS 1000
...
dprintk("%s\n", __FUNCTION__);
	...
	for (i=0; i<state->num_demod_slaves; i++) {
		struct dib7000p_state *st = state->demod_slaves[i]->demodulator_priv;
		...
		dib7000p_set_output_mode(st, OUTMODE_HIGH_Z);
 		...
 		/* maybe the parameter has been changed */
		st->sfn_workaround_active = buggy_sfn_workaround;

		if (st->demod.ops.tuner_ops.set_params)
			st->demod.ops.tuner_ops.set_params(state->demod_slaves[i], fep);
	...
	do {
		time = -1;
		for (i=0; i<state->num_demod_slaves; i++) {
			ret = dib7000p_agc_startup(state->demod_slaves[i], fep);
			if (ret > time)
				time = ret;
		}
		if (time != -1)
	...
	if (fep->u.ofdm.transmission_mode == TRANSMISSION_MODE_AUTO ||
		...
		int i, k = 0, stat = 0, num_failed = 0;

		for (i = 0; i < state->num_demod_slaves; i++)
			dib7000p_autosearch_start(state->demod_slaves[i], fep);
		
		for (i = 0; num_failed != 2 && i < MAX_AUTOSEARCH_ITERATIONS; i++) {
			for (k = 0; k < state->num_demod_slaves; k++) {
				/* dprintk("i: %d k: %d stat: %d num_failed %d\n", i, k, stat, num_failed); */
				if (!(stat & (1 << k))) { /* if not already excluded from autosearch */
					msleep(1);
					/* dprintk("%d try %d", i, k); */
					switch (dib7000p_autosearch_is_irq(state->demod_slaves[k])) {
						case 0: break;
						case 2:
						{
							u16 snr;
							dib7000p_read_snr(state->demod_slaves[k], &snr);							
							dprintk("-I-  Autosearch succeeded for demod %d, snr: %u - done.",k, snr);
#if 0							
							if (snr < 10)
							{
								num_failed++;
						...
						default:
							dprintk("-E-  Autosearch IRQ return unhandled value for demod %d.",k);
						case 1: /* fall throught wanted */
							dprintk("-I-  Autosearch failed :-) for demod %d - exluding from list.",k);
							stat |= (1 << k);
							num_failed++;
							break;
					}
		...
		dprintk("-W-  Autosearch failed for %d demod(s)",num_failed);
		return 0;  /* no channel found */
	
tune_ok:
		dib7000p_get_frontend(state->demod_slaves[k], fep); /* for the demod that worked */
		ret |= dib7000p_tune(state->demod_slaves[k], fep);
		...
		for (i = 0; i < state->num_demod_slaves; i++) {
			...
			/* this may return an error - but it is the demod that did not want to decode */
			if ( i != k ) 
				dib7000p_tune(state->demod_slaves[i], fep);
		}
...
dprintk("tune failed\n");
		...
		ret = 0;
		state->worked = 0; /* use demod 0 by default */
		for (i = 0; i < state->num_demod_slaves; i++) {
			ret |= dib7000p_tune(state->demod_slaves[i], fep);
		}
...
dprintk("tune failed\n");
	...
	for (i = 0; i < state->num_demod_slaves; i++) {
		...
		if (i == state->num_demod_slaves-1) { // last demod in a diversity chain - turn off div-in combination
dprintk("-D-  setting diversity in off for demod %d\n",i);
			dib7000p_set_diversity_in(state->demod_slaves[i], DIV_INTERNAL);
		} else {
dprintk("-D-  setting diversity in on  for demod %d\n",i);
			dib7000p_set_diversity_in(state->demod_slaves[i], DIV_BOTH);
		}

		if (i == 0) { // first demod in a diversity chain - no diversity output
dprintk("-D-  setting normal output for demod %d\n",i);
			dib7000p_set_output_mode(state, OUTMODE_MPEG2_FIFO);
		} else {
dprintk("-D-  setting diversity out on for demod %d\n",i);
			dib7000p_set_output_mode(state->demod_slaves[i]->demodulator_priv, OUTMODE_DIVERSITY);
		}
	}
...
int dib7000p_attach_slave(struct dvb_frontend *master, struct dvb_frontend *slave)
{
	struct dib7000p_state *state = master->demodulator_priv;	
	if (state->num_demod_slaves < MAX_NUM_SLAVES)
	{
		state->demod_slaves[state->num_demod_slaves++] = slave;
		return 0;
	}
...
EXPORT_SYMBOL(dib7000p_attach_slave);
