...
int ispccdc_config_size(u32 input_w, u32 input_h, u32 output_w, u32 output_h)
{
	...
	if (ispccdc_obj.ccdc_outfmt == CCDC_OTHERS_VP) {
		...
		if (cpu_is_omap3410()) {
			...
			if (ispccdc_obj.ccdc_inpfmt == CCDC_RAW) {
				omap_writel(ispccdc_obj.ccdcin_woffset
						...
						<< ISPCCDC_HORZ_INFO_NPH_SHIFT),
						ISPCCDC_HORZ_INFO);
			} else if (ispccdc_obj.ccdc_inpfmt == CCDC_YUV_BT) {
				omap_writel(0 << ISPCCDC_HORZ_INFO_SPH_SHIFT |
					(((ispccdc_obj.ccdcout_w << 1) - 1) <<
					 ISPCCDC_HORZ_INFO_NPH_SHIFT),
					ISPCCDC_HORZ_INFO);
		...
		omap_writel(((ispccdc_obj.ccdcout_w - ispccdc_obj.ccdcin_woffset) <<
					ISPCCDC_VP_OUT_HORZ_NUM_SHIFT) |
			((ispccdc_obj.ccdcout_h - (ispccdc_obj.ccdcin_hoffset + 1)) <<
					ISPCCDC_VP_OUT_VERT_NUM_SHIFT),
					ISPCCDC_VP_OUT);
		omap_writel((ispccdc_obj.ccdcin_woffset <<
					ISPCCDC_HORZ_INFO_SPH_SHIFT) |
		...
		omap_writel((ispccdc_obj.ccdcout_h - (ispccdc_obj.ccdcin_hoffset + 1)) <<
					ISPCCDC_VERT_LINES_NLV_SHIFT,
					ISPCCDC_VERT_LINES);
		ispccdc_config_outlineoffset(ispccdc_obj.ccdcout_w * 2, 0, 0);
...
{
	u32 vd1_lines = 0;
	vd1_lines = (10 << ISPCCDC_VDINT_1_SHIFT);
	omap_writel(vd1_lines, ISPCCDC_VDINT);
}
#else
		omap_writel((
				((ispccdc_obj.ccdcout_h - 2) & ISPCCDC_VDINT_0_MASK) <<
					ISPCCDC_VDINT_0_SHIFT) |
				((50 & ISPCCDC_VDINT_1_MASK) <<
		...
		omap_writel((((ispccdc_obj.ccdcout_h - 25)
				...
				ISPCCDC_VDINT);
//HTC_CSP_START
	} else if (ispccdc_obj.ccdc_outfmt == CCDC_YUV_RSZ) {
		omap_writel(0, ISPCCDC_VP_OUT);
		if (cpu_is_omap3410()) {
			omap_writel(0 << ISPCCDC_HORZ_INFO_SPH_SHIFT |
						((ispccdc_obj.ccdcout_w - 1) <<
...
void ispccdc_enable(u8 enable)
{
	if (enable) {
		if (ccdc_use_lsc && !ispccdc_obj.lsc_en &&
            ...
            ispccdc_obj.ccdc_enable = 1;
	} else {
		omap_writel(omap_readl(ISPCCDC_PCR) & ~(ISPCCDC_PCR_EN),
								ISPCCDC_PCR);
		omap_readl(ISPCCDC_PCR);
		ispccdc_obj.ccdc_enable = 0;
	}
