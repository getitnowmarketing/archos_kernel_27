...
void isppreview_config_blkadj(struct ispprev_blkadj prev_blkadj)
{
	...
	val = prev_blkadj.blue;
	val |= (prev_blkadj.green << ISPPRV_BLKADJOFF_G_SHIFT);
	val |= (prev_blkadj.red << ISPPRV_BLKADJOFF_R_SHIFT);

	omap_writel(val, ISPPRV_BLKADJOFF);
...
void isppreview_config_yc_range(struct ispprev_yclimit yclimit)
{
	...
	spin_lock(&ispprev_obj.ispprev_lock);
	val = (yclimit.maxC << ISPPRV_SETUP_YC_MAXC_SHIFT);
	val |= (yclimit.maxY << ISPPRV_SETUP_YC_MAXY_SHIFT);
	val |= (yclimit.minC << ISPPRV_SETUP_YC_MINC_SHIFT);
	val |= (yclimit.minY << ISPPRV_SETUP_YC_MINY_SHIFT);
	omap_writel(val, ISPPRV_SETUP_YC);
	spin_unlock(&ispprev_obj.ispprev_lock);
}
