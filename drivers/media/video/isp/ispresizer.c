...
omap_writel((omap_readl(ISPRSZ_CNT))&~(ISPRSZ_CNT_INPSRC|ISPRSZ_CNT_INPTYP), ISPRSZ_CNT);
	...
	case RSZ_MEM_YUV:
		cnt |= ISPRSZ_CNT_INPSRC;
		cnt &= ~ISPRSZ_CNT_INPTYP;
omap_writel((omap_readl(ISPRSZ_CNT))&~(ISPRSZ_CNT_INPTYP), ISPRSZ_CNT);
		break;
	case RSZ_MEM_COL8:
		cnt |= ISPRSZ_CNT_INPSRC;
		cnt |= ISPRSZ_CNT_INPTYP;
 ...
 **/
//HTC_CSP_START
void ispresizer_enable(u8 enable, u8 oneshot)
{
	int val;
	//printk(KERN_ERR "+ispresizer_enable()+, enable = %d, oneshot = %d\n",enable, oneshot);
