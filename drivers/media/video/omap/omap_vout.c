...
static u32 video1_numbuffers = 0; /* we use user pointer */
static u32 video2_numbuffers = 0; /* we use user pointer */
...
static inline u32 omap_vout_uservirt_to_phys(u32 virtp)
{
	...
	if (virtp >= PAGE_OFFSET) {
		...
		if (res == nr_pages) {
			...
			printk(KERN_WARNING "omap_vout_uservirt_to_phys: %08X get_user_pages failed\n", virtp);
			return 0;
		}
	}
			  ...
			  struct videobuf_buffer *vb)
{
	...
	vb->state = VIDEOBUF_PREPARED;
#else
	/* we do not use the queue, we just frob the buffer into the DSS here
	   and declare it done right away!
	*/
	int r;
	...
	addr = (unsigned long) vout->queued_buf_addr[vb->i] + vout->cropped_offset;
...
#if 0 /* MW: disable flicker filter, doubled lines on VENC look very bad */
...
//	int minor = video_devdata(file)->minor;
	...
	if (saved_v1out && saved_v1out->vfd
	    && (saved_v1out->vfd->minor == iminor(inode))) { //minor)) {
		vout = saved_v1out;
	}

	if (vout == NULL) {
		if (saved_v2out && saved_v2out->vfd
		    && (saved_v2out->vfd->minor == iminor(inode))) { //minor)) {
			vout = saved_v2out;
		}
	}
			...
			struct v4l2_buffer *buffer)
{
	...
	if (!ofh->io_allowed) {
DPRINTK("vidioc_qbuf: !ofh->io_allowed\r\n" );
		return -EINVAL;
	}
...
DPRINTK("vidioc_qbuf: %d %d %d\r\n",(V4L2_BUF_TYPE_VIDEO_OUTPUT != buffer->type),(buffer->index >= vout->buffer_allocated),(q->bufs[buffer->index]->memory != buffer->memory)   );
		return -EINVAL;
	}
	if (V4L2_MEMORY_USERPTR == buffer->memory) {
		if ((buffer->length < vout->pix.sizeimage) ||
			(0 == buffer->m.userptr)) {
DPRINTK("vidioc_qbuf: %d %d\r\n", (buffer->length < vout->pix.sizeimage),(0 == buffer->m.userptr) );
			return -EINVAL;
		}
	}
...
static unsigned long fb_reftime( struct omap_vout_device *vout, unsigned long int *next_time );
...
//printk("VIDIOC_QBUF2: %08X  %d\n", (unsigned int)buffer->m.userptr, (int)buffer->timestamp.tv_usec);
		...
		frm->droppable         = buffer->flags;
		frm->dropped           = 0;
		frm->phys_addr         = omap_vout_uservirt_to_phys(buffer->m.userptr) + vout->cropped_offset;
...
//printk ("VIDIOC_DQBUF_FLUSH:\n");
	...
	case VIDIOC_GET_REFTIME:
	{
		...
		buffer->timestamp.tv_usec = fb_reftime( vout, NULL);
//printk ("VIDIOC_GET_REFTIME: %d\n", (int)buffer->timestamp.tv_usec);
		return 0;
	}
	default:
DPRINTK ("omap_vout: unknown ioctl: %08X\n", cmd);
		/* unrecognized ioctl */
		return -ENOIOCTLCMD;
	}
}
...
//printk("apply: trans_key %d %d\n", enable, vout->win.chromakey);
...
static void get_lcd_timings(struct omap_video_timings *timings, unsigned long *rate, unsigned long *scale )
{
	unsigned xtot, ytot;
	unsigned int y_res;
	...
	ytot = y_res + timings->vfp + timings->vsw + timings->vbp;

	if( !timings->pixel_clock ) {
		// hmm, this is VENC PAL or NTSC
		if( ytot > 500 ) {
			// PAL
			*rate  = 200000;
			*scale = 4000;
...
static unsigned long fb_reftime( struct omap_vout_device *vout, unsigned long int *next_time )
{
	...
	struct omapvideo_info *ovid = &(vout->vid_info);
	struct omap_overlay *ovl = ovid->overlays[0];

	unsigned long long x;
	unsigned long long y;

	get_lcd_timings( &ovl->manager->display->panel->timings, &rate, &scale );
//DPRINTK("cnt: %8d  rate %d  scale %d\r\n", vout->vsync_cnt, rate, scale );
	
	if( !rate ) {
printk("fb_reftime error! cnt: %8d  rate %d  scale %d\r\n", vout->vsync_cnt, rate, scale );
		if( next_time ) {
			*next_time = 0;
		}
		return 0;
	...
	if( next_time ) {
		x = (vout->vsync_cnt + 1) * 1000 * (unsigned long long)scale;
		y = (unsigned long long)rate;
		do_div(x, y);
		*next_time = x;
	}
	
	x = vout->vsync_cnt * 1000 * (unsigned long long)scale;
	y = (unsigned long long)rate;
	do_div(x, y);
...
/* kilroy was here */
static void show_buffer( struct omap_vout_device *vout, unsigned long phys_addr )
{
	if (!vout->streaming) {	
printk("show_buffer: NOT STREAMING!\r\n");
	...
	omapvid_apply_changes(vout, phys_addr, 0);

	return;
}

static void vbl_irq( struct omap_vout_device *vout )
{
...
DPRINTK("    [-]           - %8lu  %d\r\n", next_time, diff );
	...
	while (!list_empty(&pending_queue)) {
		...
		if (next_pending->droppable && time_before((unsigned long)next_pending->presentation_time, venc_time ) ) {
DPRINTK("DROP[%u]  %8u < %8lu  %d\r\n", next_pending->index, next_pending->presentation_time, venc_time, diff );
			// mark this one as dropped:
			next_pending->dropped = 1;
...
void omap_vout_isr(void *arg, unsigned int irqstatus)
{
	...
	if (cur_display->type == OMAP_DISPLAY_TYPE_DPI) {
		...
		vout->nextFrm = list_entry(vout->dma_queue.next, struct videobuf_buffer, queue);
		list_del(&vout->nextFrm->queue);

		vout->nextFrm->state = VIDEOBUF_ACTIVE;

		addr = (unsigned long) vout->queued_buf_addr[vout->nextFrm->i] + vout->cropped_offset;
		r = omapvid_apply_changes(vout, addr, 0);
		if (r)
			printk(KERN_ERR VOUT_NAME "failed to change mode\n");
	} else {
		...
		if (0 == fid) {
			...
			if (list_empty(&vout->dma_queue) || (vout->curFrm != vout->nextFrm)) {
				spin_unlock(&vout->vbq_lock);
				return;
			}
			vout->nextFrm = list_entry(vout->dma_queue.next, struct videobuf_buffer, queue);
