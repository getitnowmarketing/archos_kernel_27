				   ...
				   enum v4l2_field field)
{
	...
	if (vb->state == VIDEOBUF_NEEDS_INIT) {
		/* Not needed anymore since videobuf_iolock is
			called from isp_vbq_prepare() now (if needed) */
#if 0
...
#if 1 //HTC_CSP_START
	if(vdev->vdev_sensor_config.sensor_isp == 1)
...
static int vidioc_s_fmt_cap(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	mutex_lock(&vdev->mutex);
	/* Need format setting during stream is on
		in avos. I have not seen conflicts yet */
...
/**
 * vidioc_s_fmt_sliced_vbi_cap - V4L2 set sliced vbi format
 * IOCTL handler
 * @file: ptr. to system file structure
 * @fh: ptr to hold address of omap34xxcam_fh struct (per-filehandle data)
 * @f: ptr to standard V4L2 format structure
 *
 * Attempts to set the vbi format with the sensor driver.
 * Returns the return code from vidioc_int_s_fmt_cap().
 */
static int vidioc_s_fmt_sliced_vbi_cap(struct file *file, void *fh,
				struct v4l2_format *f)
{
	struct omap34xxcam_fh *ofh = fh;
	struct omap34xxcam_videodev *vdev = ofh->vdev;
	...
	if (vdev->vdev_sensor_mode)
		/* vidioc_int_s_fmt_cap handles V4L2_BUF_TYPE_VIDEO_CAPTURE
		   and V4L2_BUF_TYPE_SLICED_VBI_CAPTURE. It's easier than
		   writing another private ioctl. */
		return vidioc_int_s_fmt_cap(vdev->vdev_sensor, &input_fmt);
	else
		return -EINVAL;
}

/**
 * vidioc_g_sliced_vbi_cap - V4L2 get sliced vbi
 * capabilities IOCTL handler
 * @file: ptr. to system file structure
 * @fh: ptr to hold address of omap34xxcam_fh struct (per-filehandle data)
 * @p: ptr to standard V4L2 sliced vbi cap structure
 *
 * Attempts to get the vbi capabilities from the sensor driver.
 * Returns the return code from vidioc_int_priv_g_sliced_vbi_cap().
 */
				...
				struct v4l2_sliced_vbi_cap *cap)
{
	...
	if (vdev->vdev_sensor_mode)
		return vidioc_int_priv_g_sliced_vbi_cap(vdev->vdev_sensor, cap);
	else
		return -EINVAL;
}
	...
	if (vdev->vdev_sensor_mode) {
		switch(vdev->slave_config[OMAP34XXCAM_SLAVE_SENSOR].cur_input) {
			case INPUT_CVBS_VI4A:
				strlcpy(inp->name, "CVBS0", sizeof(inp->name));
				inp->type = V4L2_INPUT_TYPE_TUNER;
				break;
			case INPUT_SVIDEO_VI4A_VI3A:
				strlcpy(inp->name, "SVIDEO", sizeof(inp->name));
				inp->type = V4L2_INPUT_TYPE_TUNER;
				break;
			case INPUT_SCART_VI1A_VI2A_VI3A_VI4A:
				strlcpy(inp->name, "SCART", sizeof(inp->name));
				inp->type = V4L2_INPUT_TYPE_TUNER;
				break;
			case INPUT_YPbPr_VI1A_VI2A_VI3A:
				strlcpy(inp->name, "YPbPr", sizeof(inp->name));
				inp->type = V4L2_INPUT_TYPE_TUNER;
				break;
			case INPUT_RGB_VI1A_VI2A_VI3A:
				strlcpy(inp->name, "RGB", sizeof(inp->name));
				inp->type = V4L2_INPUT_TYPE_TUNER;
				break;
			case INPUT_CVBS_VI1A:
				strlcpy(inp->name, "CVBS1", sizeof(inp->name));
				inp->type = V4L2_INPUT_TYPE_TUNER;
				break;
		}

		/* Get status (especially macrovision) */
		vidioc_int_priv_querystatus(vdev->vdev_sensor, &status);
		inp->status = status;
		inp->type = V4L2_INPUT_TYPE_TUNER;
	} else {
		if (inp->index > 0)
...
static int vidioc_g_input(struct file *file, void *fh, unsigned int *i)
{
	...
	if (vdev->vdev_sensor_mode) {
		if (vdev->slave_config[OMAP34XXCAM_SLAVE_SENSOR].cur_input
			...
			route.input = INPUT_CVBS_VI4A; /* INPUT_ANALOG_CH0 */
			route.output = 0;
		...
		switch(vdev->slave_config[OMAP34XXCAM_SLAVE_SENSOR].cur_input) {
			...
			default:
				...
				*i = vdev->slave_config[OMAP34XXCAM_SLAVE_SENSOR].cur_input;
		}
	} else
		*i = 0;
...
static int vidioc_s_input(struct file *file, void *fh, unsigned int i)
{
	...
	if (vdev->vdev_sensor_mode) {
		switch(i) {
			case INPUT_ANALOG_CH0:
				route.input = INPUT_CVBS_VI4A;
				break;
			case INPUT_SVIDEO:
				route.input = INPUT_SVIDEO_VI4A_VI3A;
				break;
			case INPUT_SCART:
				route.input = INPUT_SCART_VI1A_VI2A_VI3A_VI4A;
				break;
			case INPUT_YPbPr:
				route.input = INPUT_YPbPr_VI1A_VI2A_VI3A;
				break;
			case INPUT_RGB:
				route.input = INPUT_RGB_VI1A_VI2A_VI3A;
				break;
			case INPUT_ANALOG_CH1:
				route.input = INPUT_CVBS_VI1A;
				break;
			default:
				/* Not a pbto try "i" directly */
				route.input = i;
		}
		route.output = 0;
...
static int vidioc_default(struct file *file, void *fh, int cmd, void *arg)
{
	...
	struct flash_driver_info *data;
	struct v4l2_control vc;


	/* Handle private IOCTLs */
	switch (cmd) {
	case VIDIOC_PRIVATE_OMAP34XXCAM_SENSOR_INFO:
	...
	case VIDIOC_INT_G_VBI_DATA:
		/* Get VBI data from the sensor */
		rval = vidioc_int_priv_g_vbi_data(vdev->vdev_sensor,arg);
		goto out;
	case VIDIOC_PRIVATE_FLASH_DRIVER:
		if (vdev->vdev_sensor_config.sensor_isp) {
			rval = -EINVAL;
			goto out;
		}
		data = (struct flash_driver_info *)arg;

		vc.value = data->flash_mode;
		vc.id = V4L2_CID_FLASH_MODE;
		rval = vidioc_int_s_ctrl(vdev->vdev_flash, &vc);
	
		vc.value = data->turn_on;
		vc.id = V4L2_CID_FLASH_ENABLE;
		rval = vidioc_int_s_ctrl(vdev->vdev_flash, &vc);
