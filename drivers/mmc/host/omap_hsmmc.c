 ...
 *	Romit Dasgupta          <romit@ti.com> Race fixes
 *
...
static void mmc_clk_try_disable(struct mmc_omap_host *host)
{
	unsigned long flags;
	spin_lock_irqsave(&host->clk_lock, flags);
	if (!host->ifclks_enabled)
...
static void
mmc_omap_inact_timer(unsigned long data)
{
