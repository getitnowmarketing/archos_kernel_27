...
static int charger_flags = NO_PW_CONN;
...
static inline int twl4030charger_presence_evt(void)
{
	...
	if (ret & AC_PW_CONN) { /* If the AC charger have been connected */
...
/* FIXME-TI: not needed with new framework */
#if 0
	u8 int_src, clear = 0;
	int ret;
...
static int twl4030charger_presence(void)
{
	...
	charger_flags = (hwsts & STS_CHG) ? AC_PW_CONN : NO_PW_CONN;
	charger_flags += (hwsts & STS_VBUS) ? USB_PW_CONN : NO_PW_CONN;
...
static void twl4030_bk_bci_battery_work(struct work_struct *work)
{
	...
	schedule_delayed_work(&di->twl4030_bk_bci_monitor_work, 3000);
	return;
}
...
static void twl4030_bci_battery_work(struct work_struct *work)
{
	...
	schedule_delayed_work(&di->twl4030_bci_monitor_work, 2000);
	return;
}
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		else if (charger_flags & USB_PW_CONN)
...
int twl4030_is_ac_online(void)
{
	return (charger_flags & AC_PW_CONN) != 0;
}
EXPORT_SYMBOL_GPL(twl4030_is_ac_online);

int twl4030_is_usb_online(void)
{
	return (charger_flags & USB_PW_CONN) != 0;
}
EXPORT_SYMBOL_GPL(twl4030_is_usb_online);
...
static int __init twl4030_bci_battery_probe(struct platform_device *pdev)
{
	...
	/* enabling GPCH09 for reading backup battery voltage */
	...
	twl4030charger_hw_presence_en(ENABLE);
	...
	schedule_delayed_work(&di->twl4030_bci_monitor_work, 500);
	...
	schedule_delayed_work(&di->twl4030_bk_bci_monitor_work, 600);
...
temp_setup_fail:
	...
	twl4030charger_hw_presence_en(DISABLE);
	...
	pm_message_t state)
{
	...
	/* Zoom2 boards without debug board attached comes out of suspend
	 * due to spurious battery interrupts if no battery is connectd.
	 * This fix will disable the batery interrupts during suspend.
	 */
#ifdef CONFIG_MACH_OMAP_ZOOM2
	{
		u8 batstsmchg, batstspchg;
...
#ifdef CONFIG_MACH_OMAP_ZOOM2
	if (di->bat_int_disable_status) {
		twl4030battery_hw_level_en(ENABLE);
		twl4030battery_hw_presence_en(ENABLE);
		di->bat_int_disable_status = 0;
	}
#endif
	schedule_delayed_work(&di->twl4030_bci_monitor_work, 0);
