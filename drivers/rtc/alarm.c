...
static int alarm_ignored_on_suspend;
...
static long alarm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (ANDROID_ALARM_BASE_CMD(cmd) != ANDROID_ALARM_GET_TIME(0)) {
		...
		if (file->private_data == NULL &&
		    cmd != ANDROID_ALARM_SET_RTC && cmd != ANDROID_ALARM_IGNORE_ON_SUSPEND) {
...
from_old_alarm_set:
	...
	case ANDROID_ALARM_IGNORE_ON_SUSPEND:
		if (copy_from_user(&alarm_ignored_on_suspend,
		    (void __user *)arg, sizeof(int))) {
			rv = -EFAULT;
			goto err1;
		}
		ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_IO,
		    "alarm ignored on suspend set to %d\n",
		    alarm_ignored_on_suspend != 0 ? 1 : 0);
		break;
...
int alarm_resume(struct platform_device *pdev)
{
	...
	ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_FLOW,
			      "alarm_resume(%p)\n", pdev);
	if (alarm_enabled & ANDROID_ALARM_WAKEUP_MASK) {
		if (rtc_read_alarm(alarm_rtc_dev, &alarm) == 0 && alarm.enabled) {
			alarm.enabled = 0;
