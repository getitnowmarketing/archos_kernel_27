...
static unsigned int serial_in(struct uart_8250_port *up, int offset)
{
	...
	case UPIO_MEM16:
		return (0x00FF & readw(up->port.membase + offset));
...
static void __exit serial8250_exit(void)
{
	 ...
	 * the ports (thereby making serial8250_isa_driveare_driveromap_uarts_activer permanently
	 * in use.)
	 */
	...
	*driver8250_managed = 0x7; /* OMAP has 3 UART instances */
	...
	for (j = 0; j < UART_NR; j++) {
		...
		if ((up->port.info == NULL) ||
			!(up->port.info->flags & UIF_INITIALIZED))
			continue;
