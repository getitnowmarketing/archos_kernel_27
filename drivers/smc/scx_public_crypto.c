	...
	switch (pSession->nCryptoAlgId) {
      case PUBLIC_CRYPTO_ALG_CIPHER_AES:
		...
		if (pSession->nResultLength < pSession->nDataLength) {
         ...
         scxPublicCryptoEnableClock(PUBLIC_CRYPTO_AES_CLOCK_BIT);
      ...
      case PUBLIC_CRYPTO_ALG_CIPHER_3DES:
		...
		if (pSession->nResultLength < pSession->nDataLength) {
         ...
         scxPublicCryptoEnableClock(PUBLIC_CRYPTO_DES_CLOCK_BIT);
      ...
      case PUBLIC_CRYPTO_ALG_DIGEST_SHA256:
         ...
         scxPublicCryptoEnableClock(PUBLIC_CRYPTO_SHA_CLOCK_BIT);
...
#else
   SetPageLocked(virt_to_page(pSession->pInnerBuffer));
#endif
...
#else
         ClearPageLocked(virt_to_page(pSession->pInnerBuffer));
#endif
   ...
   case PUBLIC_CRYPTO_ALG_CIPHER_AES:
      ...
      dprintk(KERN_INFO "pPubCrypto->nAesSession(%u)\n",pPubCrypto->nAesSession);
		if (pPubCrypto->nAesSession == 1) {
         scxPublicCryptoDisableClock(PUBLIC_CRYPTO_AES_CLOCK_BIT);
         dprintk(KERN_INFO "\n\n\n\n DISABLE AES \n\n\n\n");
      }
      pPubCrypto->nAesSession --;
      break;
   ...
   case PUBLIC_CRYPTO_ALG_CIPHER_3DES:
      ...
      dprintk(KERN_INFO "pPubCrypto->nDesSession(%u)\n",pPubCrypto->nDesSession);
		if (pPubCrypto->nDesSession == 1) {
         scxPublicCryptoDisableClock(PUBLIC_CRYPTO_DES_CLOCK_BIT);
         dprintk(KERN_INFO "\n\n\n\n DISABLE DES \n\n\n\n");
      }
      pPubCrypto->nDesSession --;
      break;
   ...
   case PUBLIC_CRYPTO_ALG_DIGEST_SHA256:
      ...
      dprintk(KERN_INFO "pPubCrypto->nShaSession(%u)\n",pPubCrypto->nShaSession);
		if (pPubCrypto->nShaSession == 1) {
         scxPublicCryptoDisableClock(PUBLIC_CRYPTO_SHA_CLOCK_BIT);
         dprintk(KERN_INFO "\n\n\n\n DISABLE SHA \n\n\n\n");
      }
      pPubCrypto->nShaSession --;
      break;
...
void scxPublicCryptoEnableHwClock(U32 vClockBit)
{
   U32* pClockReg;
   U32* pAutoIdleReg;

   pClockReg    = (U32*) IO_ADDRESS(PUBLIC_CRYPTO_HW_CLOCK_ADDR);
   pAutoIdleReg = (U32*) IO_ADDRESS(PUBLIC_CRYPTO_HW_AUTOIDLE_ADDR);
...
void scxPublicCryptoDisableHwClock(U32 vClockBit)
{
   U32* pClockReg;
   U32* pAutoIdleReg;
...
void scxPublicCryptoEnableClock(uint32_t vClockBit)
{
   ...
   scxPublicCryptoEnableHwClock(vClockBit);
}
...
void scxPublicCryptoDisableClock(uint32_t vClockBit)
{
   ...
   scxPublicCryptoDisableHwClock(vClockBit);
}
