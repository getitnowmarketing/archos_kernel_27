...
U32 scxPublicDMAInit(void)
{
   ...
   /*
      Allows 1 HW thread reservation for priority channels (secure channels
      have their priorities set by the PA (either read CCR[6] or
      write CCR[26]) )
   */
   SETREG32(&g_pDmaController->DMA4_GCR, ((0x01)<<12));
...
{
   U32 timeoutCounter = DMA_TIMEOUT_CONST;

	while (!(INREG32(&(g_pDmaController->CHNL_CTRL[lch].CSR)) & DMA_CSR_Is_Finished) && (--timeoutCounter) != 0) {
   ...
   /* If timeout occurred */
	if (timeoutCounter == 0) {
		dprintk(KERN_ERR "scxPublicDMAPoll(%d) : Timeout\n", lch);
