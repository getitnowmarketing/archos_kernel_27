      ...
      u32 nFullCleanup)
{
   ...
   /* check pShmemDesc contains a descriptor */
	...
	nDownInterruptibleReturn = down_interruptible(&(pShmemMonitor->sharedMemoriesMutex));
       ...
       */
		if (current->euid != 0 && current->egid != 0) {
         dprintk(KERN_ERR "SCXLNXConnOpenClientSession: "
               " user %d not allowed to open session with S_LOGIN_PRIVILEGED_n", current->uid);
         nError = -EACCES;
         goto exit;
      }
