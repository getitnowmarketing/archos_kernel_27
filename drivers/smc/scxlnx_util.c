      ...
      SCX_UUID *pIdentifier)
{
   ...
   pIdentifier->clock_seq_and_node[0] = (pHashData[8] & 0x3F) | 0x40;
   ...
   down_read(&(mm->mmap_sem));
	for (vma = mm->mmap; vma != NULL; vma = vma->vm_next) {
		if ((vma->vm_flags & VM_EXECUTABLE) != 0 && vma->vm_file != NULL) {
         ...
         sha1_update(&sha1Context, (const u8 *)&(current->uid), sizeof(current->uid));
         sha1_update(&sha1Context, (const u8 *)&(current->euid), sizeof(current->euid));
