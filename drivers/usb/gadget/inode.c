...
static unsigned int _mtp_userspace_status = 0x2001;
...
// Debug for the write submit/cancel ioctl
#define DBG1 if(0) 
...
struct ep_data {
	...
	struct usb_ep			*ep;
	struct usb_request		*req;
	// Asynchronous I/O using ioctls
	struct gadget_io_buffer		*io_buf;
...
static void _io_complete(struct usb_ep *ep, struct usb_request *req)
{
	struct ep_data *data = (struct ep_data*)req->context;

	data->io_buf->transferred = req->actual;

DBG1{
	printk(KERN_ERR"io_complete %p (%d) (transferred : %d)", req, req->status, data->io_buf->transferred);
	if( data->io_buf->direction == GADGETFS_OUT )
		printk("(%02x %02x %02x %02x)\n", data->io_buf->kdata[0], data->io_buf->kdata[1], data->io_buf->kdata[2], data->io_buf->kdata[3]);
	else
		printk("\n");
}

	data->io_buf->status = GADGETFS_IO_DONE;

	complete(&data->done);
}

static int _io_submit(struct ep_data *data)
{
	struct usb_request *req = data->io_buf->req;

	if (unlikely (data->ep == NULL)) {
		return -ENODEV;
	}
	...
	req->context	= data;
	req->complete	= _io_complete;
	req->buf	= data->io_buf->kdata;
	req->length	= data->io_buf->length;
	req->zero	= data->io_buf->zero;

	data->io_buf->status 	= GADGETFS_IO_PENDING;

DBG1{
	printk(KERN_ERR"queue %p (%d bytes)", req, req->length);
	if( data->io_buf->direction == GADGETFS_IN )
		printk("(%02x %02x %02x %02x)\n", data->io_buf->kdata[0], data->io_buf->kdata[1], data->io_buf->kdata[2], data->io_buf->kdata[3]);
	else
		printk("\n");
}
	return usb_ep_queue (data->ep, req, GFP_ATOMIC);
}

static int _io_cancel(struct ep_data *data)
{
	int err = 0;

	printk(KERN_ERR "dequeue %p (transferred : %d)\n", data->io_buf->req, ((struct usb_request *)data->io_buf->req)->actual);

	if (unlikely (data->ep == NULL)) {
		return -ENODEV;
	}

	err = usb_ep_dequeue (data->ep, (struct usb_request *)data->io_buf->req);

	return err;
}

static struct gadget_io_buffer * alloc_gadget_buffer( struct usb_ep *ep, int size )
{
	struct gadget_io_buffer *buf;

	buf = kmalloc(sizeof(struct gadget_io_buffer), GFP_KERNEL);

	if( !buf ){
DBG1		printk(KERN_ERR "no buf :-(\n");
		return NULL;
	}

	buf->size = size;
	buf->req  = usb_ep_alloc_request(ep, GFP_KERNEL);

	//printk(KERN_ERR"request allocated @%p", buf->req);

	if( !buf->req ){
DBG1		printk(KERN_ERR "no req :-(\n");
		kfree(buf);
		return NULL;
	}

	buf->kdata = kmalloc(size, GFP_KERNEL);

	if( !buf->kdata ){
DBG1		printk(KERN_ERR "no kdata :-( %d\n", size);
		usb_ep_free_request(ep, buf->req);
		kfree(buf);
		return NULL;
	}
...
static void free_gadget_buffer( struct usb_ep *ep, struct gadget_io_buffer *buf )
{
	if( buf->req )
		usb_ep_free_request(ep, buf->req);
	if( buf->kdata )
		...
		unsigned code, unsigned long value)
{
	...
	if (likely (data->ep != NULL)) {
		...
		case GADGETFS_CLEAR_HALT:
			...
			break;
		case GADGETFS_CLEAR_QUEUE:
DBG1			printk( KERN_INFO "GADGETFS_CLEAR_QUEUE\n");
			// NOTE : clearing fifo doesn't seem necessary anymore
// 			usb_ep_fifo_clear(data->ep);
			break;
		case GADGETFS_IO_SUBMIT:
		{
			struct gadget_io_buffer *buf = data->io_buf;
			struct gadget_io_buffer temp;
DBG1			printk( KERN_INFO "GADGETFS_IO_SUBMIT\n");

			spin_unlock_irq (&data->dev->lock);

			if (copy_from_user(&temp, (void __user *)value, sizeof(struct gadget_io_buffer))){
DBG1				printk( KERN_INFO "CANNOT GET USER DATA\n");
				status = -EFAULT;
				goto exit;
			}
			...
			buf->direction	= temp.direction;
			buf->zero	= temp.zero;

			if( buf->length > buf->size ){
DBG1				printk( KERN_INFO "NO MEM\n");
				status = -ENOMEM;
				goto exit;
			}

			if (buf->direction == GADGETFS_IN) {
				if (copy_from_user(buf->kdata, (void __user *)buf->data, buf->length)){
DBG1					printk( KERN_INFO "CANNOT GET USER DATA2\n");
					status = -EFAULT;
					spin_lock_irq (&data->dev->lock);
					goto exit;
				}
			...
			status = _io_submit(data);
			// FALLTHROUGH
		}
		case GADGETFS_IO_GET_STATUS:
		{
			struct gadget_io_buffer *buf = data->io_buf;
DBG1			printk( KERN_INFO "GADGETFS_IO_GET_STATUS\n");

			spin_unlock_irq (&data->dev->lock);

			// We're not done --> sleep a bit 
			if( buf->status != GADGETFS_IO_DONE ){

				struct timespec t;
				t.tv_sec  = 0;
				t.tv_nsec = 100 * 1000 * 1000;
				// 1s timeout;
				wait_event_interruptible_timeout(data->done.wait, data->done.done, timespec_to_jiffies(&t) + 1);
			}

			if( buf->status == GADGETFS_IO_DONE ){
				// done: return status & copy data back
DBG1				printk( KERN_INFO "IO_DONE\n");
				if (buf->direction == GADGETFS_OUT) {
					if (copy_to_user((void __user *)buf->data, buf->kdata, buf->length)){
						status = -EFAULT;
						spin_lock_irq (&data->dev->lock);
						goto exit;
					}
			...
			if (copy_to_user((void __user *)value, buf, sizeof(struct gadget_io_buffer))){
				status = -EFAULT;
				spin_lock_irq (&data->dev->lock);
				goto exit;
			}
		...
		case GADGETFS_IO_CANCEL:
		{
			struct gadget_io_buffer temp;
			printk( KERN_INFO "GADGETFS_IO_CANCEL\n");

			if( data->io_buf->status == GADGETFS_IO_DONE ){
				printk( KERN_INFO "allready done\n");
			}
			...
			status = _io_cancel(data);

			printk( KERN_INFO "cancel status %d\n", status);
			break;
		}
		case GADGETFS_IO_DEVICE_STATUS:
		{
			int temp = 1;
			...
			if ( temp ) {
				_mtp_userspace_status = 0x2001;
			} else {
				_mtp_userspace_status = 0x2019;
			}
...
next_event (struct dev_data *dev, enum usb_gadgetfs_event_type type)
{
	...
	case GADGETFS_SUSPEND:		/* same effect */
		...
		for (i = 0; i != dev->ev_next; i++) {
			// Don't timeout CANCEL REQUEST events
			if (dev->event [i].type != type || (dev->event[i].u.setup.bRequest == 0x64) || (dev->event[i].u.setup.bRequest == 0x67))
				continue;
...
gadgetfs_setup (struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
	...
	case 0x67:
		if (ctrl->bRequestType != 0xa1)
			break;
 		dev->setup_out_ready = 0;
		setup_req (dev->gadget->ep0, req, 4 * sizeof(char));

		ptr = (char *)req->buf;
		ptr[0] =  LSB(4);
		ptr[1] =  MSB(4);
		ptr[2] =  LSB(_mtp_userspace_status);
		ptr[3] =  MSB(_mtp_userspace_status);
		...
		req->zero = value < w_length;
 		...
 		printk( KERN_INFO "gadgetfs_setup : received GET DEVICE STATUS, status is : %04X\n", _mtp_userspace_status);
...
delegate:
			...
			if (unlikely (!dev->setup_in && w_length)) {
				value = setup_req (gadget->ep0, dev->req, w_length);
				if (value < 0)
					break;

				value = usb_ep_queue (gadget->ep0, dev->req, GFP_ATOMIC);
	...
	if ( ( ctrl->bRequest != 0x67 ) && ( ( value >= 0 ) ) && ( dev->state != STATE_DEV_SETUP ) ) {
		req->length = value;
		req->zero = value < w_length;
		value = usb_ep_queue (gadget->ep0, req, GFP_ATOMIC);
		if (value < 0) {
...
restart:
	...
	list_for_each_safe (entry, tmp, &dev->epfiles) {
		...
		free_gadget_buffer (ep->ep, ep->io_buf);
		ep->ep = NULL;
		wake_up (&ep->wait);
		put_ep (ep);
...
static int activate_ep_files (struct dev_data *dev)
{
	...
	gadget_for_each_ep (ep, dev->gadget) {
		...
		data->io_buf = alloc_gadget_buffer (ep, 32 * 1024);
		if (!data->io_buf)
			goto enomem0;
