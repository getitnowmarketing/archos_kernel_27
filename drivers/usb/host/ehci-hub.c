...
static int ehci_bus_suspend (struct usb_hcd *hcd)
{
	...
	/* Patch from mainline: "USB: EHCI: ensure all watchdog timer events are deleted when suspending usb" */
...
#if 0
	/* MOT: Workaround for panic-mode re-enumeration
	 * Also needed since the OMAP3 does not support dynamic handoff
	 * of a port to OHCI
	 */
...
ehci_hub_status_data (struct usb_hcd *hcd, char *buf)
{
	...
	for (i = 0; i < ports; i++) {
		...
		/* Some HCs don't like to be jostled during a reset */
		if (ehci->reset_done[i] &&
				time_before(jiffies, ehci->reset_done[i])) {
			printk("Skip status poll for port %d\n", i+1);
			continue;
		}
		temp = ehci_readl(ehci, &ehci->regs->port_status [i]);
