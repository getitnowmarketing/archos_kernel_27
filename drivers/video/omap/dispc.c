...
static struct {
	...
	struct clk	*dss_ick, *dss1_fck, *dss2_fck;
...
{
	/*pr_debug("dispc_write_reg %p - %08x\n", dispc.base+idx, val);*/
...
{
	pr_debug("omap_dispc_set_lcd_size %ix%i\n", x, y);
...
{
	pr_debug("omap_dispc_set_digit_size %ix%i\n", x, y);
...
{
	pr_debug("omap_dispc_enable_lcd_out %i\n", enable);
...
{
	pr_debug("omap_dispc_enable_digit_out %i\n", enable);
...
static void switch_to_dsipll_clk_source (void) 
{
	u32 val;
	
	pr_debug("switch_to_dsipll_clk_source\n");
	
	/* Switch DISPC FCLK to DSI PLL HS divider */
	val = omap_readl(DSS_BASE + DSS_CONTROL);
	val = val | (1<<1) | (1<<0);
	omap_writel(val, DSS_BASE + DSS_CONTROL);
}
				  ...
				  int color_mode)
{
	...
	cconv_en = 0;
	alpha_en = 0;
	alpha_shift = 18 + chout_val;
	
	switch (color_mode) {
	case OMAPFB_COLOR_ARGB32:
		color_code = DISPC_ARGB_32_BPP;
		alpha_en = 1;
		bpp = 32;
		break;
	case OMAPFB_COLOR_RGBA32:
		color_code = DISPC_RGBA_32_BPP;
		alpha_en = 1;
		bpp = 32;
		break;
	...
	dispc_write_reg(DISPC_GLOBAL_ALPHA, 0x00FF00FF);
	MOD_REG_FLD(DISPC_CONFIG, FLD_MASK(18, 2), (alpha_en << alpha_shift));	
				  ...
				  int color_mode)
{
	...
	pr_debug("omap_dispc_setup_plane\n");
...
static int omap_dispc_enable_plane(int plane, int enable)
{
	...
	pr_debug("omap_dispc_enable_plane %i, %i\n", plane, enable);
...
static int omap_dispc_commit(struct omapfb_device *fbdev, 
		enum omapfb_channel_out channel_out)
{
	unsigned int mask;
	
	pr_debug("omap_dispc_commit %i\n", channel_out);

	if (channel_out == OMAPFB_CHANNEL_OUT_LCD)
		mask = (1 << 5);
	else if (channel_out == OMAPFB_CHANNEL_OUT_DIGIT)
		mask = (1 << 6);
	else
		return -EINVAL;
	
	MOD_REG_FLD(DISPC_CONTROL, mask, mask);
...
static int omap_dispc_set_update_mode(enum omapfb_update_mode mode)
{
	...
	pr_debug("omap_dispc_set_update_mode\n");
...
static void omap_dispc_get_caps(int plane, struct omapfb_caps *caps)
{
	...
	if (plane == 0)
		caps->plane_color |= (1 << OMAPFB_COLOR_CLUT_8BPP) |
				     ...
				     (1 << OMAPFB_COLOR_ARGB32) |
				     (1 << OMAPFB_COLOR_RGBA32) |
...
static void __calc_ck_div(int is_tft, 
		int pck, int fck, int *lck_div, int *pck_div)
{
	unsigned long lck;

	pr_debug("calc_ck_div\n");
...
static u32 calc_clkrate(u32 pixclock)
{
	...
	pr_debug("omap_disp_get_clkrate = %d -> %d\n", pixclock, clkrate);
	return clkrate;
}
...
static void set_lcd_timings(void)
{
	...
	/* 
	 * if we use the DSI PLL instead of dss1_fck, we are free
	 * to set the frequency more exactly to what we need
	 */
	if (dispc.use_dsi_pll) {
		u32 pll_clkrate;

		/* TODO: set DSI PLL to a multiple of the desired
		 * pixel clock and calculate lck_div, pck_div
		 */
		pll_clkrate = calc_clkrate(panel->pixel_clock);
		omap_disp_start_dsi_pll(pll_clkrate);
		__calc_ck_div(is_tft, panel->pixel_clock * 1000, pll_clkrate * 1000,
				&lck_div, &pck_div);
	} else
		calc_ck_div(is_tft, panel->pixel_clock * 1000, &lck_div, &pck_div);

	pr_debug("setting pixel clock divisor lck=%i pck=%i\n", 
			lck_div, pck_div);
...
static int get_dss_clocks(void)
{
	...
	char *dss2_fck = "dss2_alwon_fck";
	char *tv_fck = cpu_is_omap34xx() ? "dss_tv_fck" : "dss_54m_fck";

	pr_debug("get_dss_clocks\n");
	
	if (IS_ERR((dispc.dss_ick = clk_get(dispc.fbdev->dev, dss_ick)))) {
		dev_err(dispc.fbdev->dev, "can't get %s\n", dss_ick);
	...
	if (dispc.use_dsi_pll) {
		if (IS_ERR((dispc.dss2_fck = clk_get(dispc.fbdev->dev, dss2_fck)))) {
			dev_err(dispc.fbdev->dev, "can't get %s\n", dss2_fck);
			clk_put(dispc.dss_ick);
			clk_put(dispc.dss1_fck);
			clk_put(dispc.dss_54m_fck);
			return PTR_ERR(dispc.dss2_fck);
		}
	}
