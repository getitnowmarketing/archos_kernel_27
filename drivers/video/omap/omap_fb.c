...
#else
#define omap24xxfb_fb_size(rotation) \
	rotation ? (2048 * 640 * (16/8)) : (800 * 480 * (32/8) * 2)
#endif
/* 800 x 480 is the max framebuffer size we allow for WVGA LCD @ 32bpp */
...
/*
 *	omap24xxfb_get_flipbuffers() - Returns the number of flipping buffers
 *	that can be used, based on the memory that is available.
 *	@info: frame buffer structure that represents a single frame buffer
 *	@var: frame buffer variable screen structure
 *
 */
static int omap24xxfb_get_flipbuffers(struct fb_info *info,
		struct fb_var_screeninfo *var) {
	int ret = 0, i = 0;

	/* checks what would be the maximum number of flipping buffers */
	for (i = FB_MAX_FLIPBUF ; i >= 0 ; i--) {
		var->yres_virtual = var->yres * i;
	...
	printk(KERN_DEBUG "omap24xxfb_get_flipbuffers::"
			"flipping buffers =%d \n", ret);
	return ret;
}
...
omap24xxfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	switch (var_to_depth(&v)) {
		case 1:
			v.yres_virtual = (!oinfo->rotation_support ?
				v.yres * omap24xxfb_get_flipbuffers(info, &v) :
				v.yres_virtual);
		...
		case 24:
			/*
			* Max supported buffers: 2 buffers-800x480 @ 24bpp
			* becuase of memory restrictions
			* virtual y_res is set accordingly
			*/
			v.yres_virtual = (!oinfo->rotation_support ?
				v.yres * omap24xxfb_get_flipbuffers(info, &v) :
				v.yres_virtual);
		...
		case 32:
			/*
			* Max supported buffers: 2 buffers-800x480 @ 32bpp
			* becuase of memory restrictions
			* virtual y_res is set accordingly
			*/
			v.yres_virtual = (!oinfo->rotation_support ?
			...
			v.green.offset = 16;
			v.blue.offset = 8;
			v.red.length = v.blue.length = v.green.length =
						v.transp.length = 8;
			v.transp.offset =  0;
			break;
		default:
...
{
	struct omap24xxfb_info *omapfb = container_of(es,
					 struct omap24xxfb_info, early_suspend);

	acquire_console_sem();
	fb_set_suspend(&omapfb->info, 1);
	release_console_sem();
	...
	omap24xxfb_blank(1, &omapfb->info);
	awake = 0;
	omapfb->asleep = 1;
}

static void omapfb_resume(struct early_suspend *es)
{
	struct omap24xxfb_info *omapfb = container_of(es,
					 struct omap24xxfb_info, early_suspend);
	...
	/* unblank the screen */
	omap24xxfb_blank(0, &omapfb->info);
	awake = 1;
	acquire_console_sem();
	fb_set_suspend(&omapfb->info, 0);
	release_console_sem();
}
#endif
...
int __init omap24xxfb_init(void)
{
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	oinfo->early_suspend.suspend = omapfb_early_suspend;
	oinfo->early_suspend.resume  = omapfb_resume;
	oinfo->early_suspend.level   = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
	register_early_suspend(&oinfo->early_suspend);
#endif
	...
	printk( KERN_DEBUG "Frame buffer -- address = 0x%x \n",
			(int)oinfo->fb_base);
	...
	DBGLEAVE;
	omap2_disp_register_synclost_isr();
	omap2_disp_register_gfx_fifo_underflow_isr();
	return 0;
