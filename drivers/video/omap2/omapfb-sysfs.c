		...
		char *buf)
{
	...
	enum omap_dss_venc_norm tv_norm;
	enum omap_dss_venc_type tv_type;
	...
	for (i = 0; i < fbdev->num_displays; i++) {
		...
		if (display->get_tv_norm)
			tv_norm = display->get_tv_norm(display);
		else
			tv_norm = -1;

		if (display->get_tv_type)
			tv_type = display->get_tv_type(display);
		else
			tv_type= -1;
		
		l += snprintf(buf + l, size - l,
				"%s e:%d u:%d t:%d h:%u/%u/%u/%u "
				"v:%u/%u/%u/%u l:%u p:%u r:%d i:%d n:%d s:%d\n",
				display->name,
 ...
 */
#define OMAP2FB_DEF_SLEEP_TIMEOUT (0)
