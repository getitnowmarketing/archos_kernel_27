...
#ifdef CONFIG_ANDROID_PARANOID_NETWORK
static inline int in_group_net_bt_admin(void)
{
	return in_egroup_p(AID_NET_BT_ADMIN);
}
# else
static inline int in_group_net_bt_admin(void)
{
...
static int hci_sock_ioctl(struct socket *sock, unsigned int cmd, unsigned long arg)
{
	...
	case HCIDEVUP:
		if (!capable(CAP_NET_ADMIN) && !in_group_net_bt_admin())
			return -EACCES;
	...
	if (bt_cb(skb)->pkt_type == HCI_COMMAND_PKT) {
		...
		if (((ogf > HCI_SFLT_MAX_OGF) ||
				!hci_test_bit(ocf & HCI_FLT_OCF_BITS, &hci_sec_filter.ocf_mask[ogf])) &&
					!capable(CAP_NET_RAW) && !in_group_net_bt_admin()) {
			err = -EPERM;
			goto drop;
		}
		...
		if (!capable(CAP_NET_RAW) && !in_group_net_bt_admin()) {
			err = -EPERM;
			goto drop;
		}
