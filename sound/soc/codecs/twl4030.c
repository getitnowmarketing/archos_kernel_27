		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	/* Set master FM loop switch when any bypass */
	reg = twl4030_read_reg_cache(w->codec, TWL4030_REG_MISC_SET_1);
	if (twl4030->bypass_state)
		reg |= TWL4030_FMLOOP_EN;
	else
		reg &= ~TWL4030_FMLOOP_EN;
	twl4030_write(w->codec, TWL4030_REG_MISC_SET_1, reg);
...
static int omap_fm_hw_params(struct snd_pcm_substream *substream,
			   struct snd_pcm_hw_params *params,
			   struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
...
static int omap_fm_set_dai_sysclk(struct snd_soc_dai *codec_dai,
		int clk_id, unsigned int freq, int dir)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	return 0;
...
static int omap_fm_set_dai_fmt(struct snd_soc_dai *codec_dai,
			     unsigned int fmt)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	return 0;
...
struct snd_soc_dai twl4030_dai[] = {
...
#ifdef CONFIG_SND_SOC_TWL4030_FM
	{
		...
		.capture = {
			...
			.formats = TWL4030_FORMATS,},
		.ops = {
			.hw_params = omap_fm_hw_params,
			.set_sysclk = omap_fm_set_dai_sysclk,
			.set_fmt = omap_fm_set_dai_fmt,
		}
	}
#endif
};
