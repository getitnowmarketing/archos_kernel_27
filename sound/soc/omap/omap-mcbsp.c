				  ...
				  struct snd_soc_dai *dai)
{
	...
	printk(KERN_DEBUG "omap_mcbsp_dai_trigger: %i\n", cmd);
				    ...
				    struct snd_soc_dai *dai)
{
	...
	/* Do we are in SPDIF mode ? */
	switch (mcbsp_data->fmt & SND_SOC_DAIFMT_FORMAT_MASK) {
	case SND_SOC_DAIFMT_SPDIF:
		/* 32 bit DMA breaks SPDIF/HDMI... */
		omap_mcbsp_dai_dma_params[id][substream->stream].type = 16;
		spdif = 1;
		break;
	default:
		/* 32 bit DMA is ok in audio mode, but channels needs to be swapped */
		omap_mcbsp_dai_dma_params[id][substream->stream].type = 32;
	}
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		/* Set word lengths */
		wlen = 16;
		switch (omap_mcbsp_dai_dma_params[id][substream->stream].type) {
		case 32:
	...
	if (spdif) {
		/* The recording has to work even if
			the output is in SPDIF mode */ 
		...
		/* Don't care about channels number and frame
			length 2. Set 8 frames (frame length 1) */
		regs->xcr2	|= XFRLEN2(1 - 1);
		regs->xcr1	|= XFRLEN1(8 - 1);		
	} else {
		switch (channels) {
		case 2:
			regs->xcr2	&= ~(XPHASE);
...
{								\
	...
	.playback = {						\
		...
		.formats = SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S32_LE | SNDRV_PCM_FMTBIT_IEC958_SUBFRAME_LE,		\
	},							\
	.capture = {						\
		.channels_min = 1,				\
		.channels_max = 2,				\
