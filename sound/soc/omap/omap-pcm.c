...
static int omap_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	printk(KERN_DEBUG "omap_pcm_trigger: %i\n", cmd);
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		ret = omap_request_dma(dma_data->dma_req, dma_data->name,
				       omap_pcm_dma_irq, substream, &prtd->dma_ch);
		if (!ret && !cpu_is_omap1510()) {
			/*
			 * Link channel with itself so DMA doesn't need any
			 * reprogramming while looping the buffer
			 */
			printk(KERN_DEBUG "linking dma channel %i\n", prtd->dma_ch);
			omap_dma_link_lch(prtd->dma_ch, prtd->dma_ch);
			omap_pcm_prepare(substream);
			prtd->period_index = 0;
			omap_start_dma(prtd->dma_ch);
		} else
			printk(KERN_ERR "SNDRV_PCM_TRIGGER_RESUME: failed, ret=%i\n", ret);
		break;
